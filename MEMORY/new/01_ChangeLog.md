...
2026-06-10  配置回退        BS=64 EPOCH=10 LR_SCHEDULE=constant   ← 恢复
2026-06-10  YOLOv8s→YOLOv8l 尝试  → FAILED（yolo val bug，Pretrained mAP=0%，训练正常但指标失真）
```
...
3. 但 `yolo.save()` 导出的 ultralytics ckpt 格式存在以下问题：
   - 检测头的 `f` 属性（层索引引用）被丢弃
   - 子进程 `val()` 时报错 `AttributeError: 'Detect' object has no attribute 'f'`，导致 val 崩溃
   - 即使绕过崩溃，模型 `names` 仍是 COCO 80 类名，而非 VisDrone 10 类

**次要问题（已修复但不足以解决问题）**：
- `yolo_wrapper.py`：删除了 `new_head.training = True`（导致 val 时跳过 NMS）
- `fl_core.py`：子进程在 temp 目录关闭前运行（已修复）
- `fl_core.py`：不再重复 forward 算 test_loss（避免 OOM）

**根本结论**：`fl_core.py` 的子进程 YOLO val 方案存在 ultralytics 内部机制冲突，不可靠。
...
**关键症状**：差配置 R30 每客户端 epoch 损失 ~87-94，三轮几乎无变化——后 15 轮模型已停止学习。
