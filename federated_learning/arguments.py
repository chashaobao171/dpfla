...
from federated_learning.utils.losses import get_loss_function
...
class Arguments:
    ...
    def __init__(self, logger):
        ...
        self.lr = 0.0001  # 极小学习率，几乎不破坏预训练权重
        self.momentum = 0.9
    ...
    def update_loss_function_for_dataset(self):
        """根据当前数据集类型更新损失函数"""
        # 对于VisDrone使用完整YOLO损失，其他数据集使用标准损失
        use_full_yolo = (self.dataset_name == 'VisDrone')
        self.loss_function = get_loss_function(self.dataset_name, self.num_classes, use_full_yolo_loss=use_full_yolo)
